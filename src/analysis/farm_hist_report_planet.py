...
if __name__ == '__main__':
    ...
    OUTPUT_PATH = os.path.join(CURRENT_DIR, f"farm_hist_report_{village_name}_planet.pdf")
    ...
    columns = [f"{months_names[month[1]]}_{month[0]}_crop_presence" for month in cycle_months]
    columns_str = ", " .join(columns)

    # Add cropping pattern column
    crop_pattern_column = f"crop_cycle_{year}_{year+1}"
    ...
    with PdfPages(OUTPUT_PATH) as pdf:
        ...
        for i, month in enumerate(cycle_months):
            ...
            area_with_crop = gdf[gdf[columns[i]] > 0.5].area.sum() / 10000  # Convert to hectares
            area_without_crop = gdf[gdf[columns[i]] <= 0.5].area.sum() / 10000  # Convert to hectares
            total_area = round(gdf.area.sum() / 10000, 5)  # Convert to hectares
