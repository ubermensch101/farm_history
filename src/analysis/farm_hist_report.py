...
if __name__ == '__main__':
    ...
    table_config = config.setup_details["tables"]["villages"][0]

    cycle_months = config.setup_details["months"]["agricultural_months"]
    year = config.setup_details["months"]["agricultural_months"][0][0]
    OUTPUT_PATH = os.path.join(CURRENT_DIR, f"farm_hist_report_{year}.pdf")
    ...
    with PdfPages(OUTPUT_PATH) as pdf:
        # Create title page
        title_fig, title_ax = plt.subplots(figsize=(8.27, 11.69))  # A4 dimensions
        title_ax.axis('off')
        ...
        for i, month in enumerate(cycle_months):
            fig = plt.figure(figsize=(8.27, 11.69))  # A4 dimensions
            ax_plot = fig.add_axes([0.1, 0.45, 0.8, 0.5])  # Larger plot area
            ...
            ax_plot.legend(handles, legend_labels, title='Crop Presence Probability', loc='upper right')

            # Crop presence statistics
            num_farms_with_crop = len(gdf[gdf[columns[i]] > 0.5])
            num_farms_without_crop = len(gdf[gdf[columns[i]] <= 0.5])
            area_with_crop = gdf[gdf[columns[i]] > 0.5].area.sum()  
            area_without_crop = gdf[gdf[columns[i]] <= 0.5].area.sum()
            total_area = round(gdf.area.sum(), 5) 
            area_with_crop = round(area_with_crop, 6)
            area_without_crop = round(area_without_crop, 6)  
            percent_area_with_crop = round((area_with_crop / total_area) * 100, 5)  # Clip to 5 decimal places

            # Create a table subplot
            table_data = {"# Farms with crop": num_farms_with_crop,
                          "# Farms without crop": num_farms_without_crop,
                          "Area with crop (sq. m)": area_with_crop,
                          "Area without crop (sq. m)": area_without_crop,
        ...
        crop_pattern_counts = gdf['crop_pattern'].value_counts()
        ...
        pattern_fig = plt.figure(figsize=(8.27, 11.69))  # A4 dimensions
        pattern_ax_plot = pattern_fig.add_axes([0.1, 0.45, 0.8, 0.5])  # Larger plot area
        ...
        pattern_ax_plot.legend(handles, labels, title='Cropping Pattern', loc='upper right')
