...
if __name__=='__main__':
    ...
    evalscript_ir_cloudmask = """
        ...
        function evaluatePixel(sample) {
            return [3.5*sample.B04, 3.5*sample.B03, 3.5*sample.B02, 3.5*sample.B08, sample.CLM];
        }
    """
    ...
    for year in years:
        ...
        for i, interval in enumerate(intervals):
            ...
            data_folder = os.path.join(ROOT_DIR, f"{interval_type}_cloud", table["table"], f"{year}", f"{interval[2]}")    
            if os.path.exists(data_folder):
                print("Data already exists for this interval! Skipping...")
            else:
                request = SentinelHubRequest(
                    data_folder=data_folder,
                    evalscript=evalscript_ir_cloudmask,
                    input_data=[
