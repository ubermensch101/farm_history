...
if __name__=='__main__':
    ...
    for i in range(interval_length):
        ...
        drop column if exists hue_mean_{args.year}_{args.interval}_{i+1};
        alter table {table["schema"]}.{table["table"]}
        drop column if exists hue_stddev_{args.year}_{args.interval}_{i+1};
        """
    ...
    for i in range(interval_length):
        sql_query = f"""
        alter table {table["schema"]}.{table["table"]}
        add column hue_mean_{args.year}_{args.interval}_{i+1} numeric;
        alter table {table["schema"]}.{table["table"]}
        add column hue_stddev_{args.year}_{args.interval}_{i+1} numeric;
        """
        with pgconn.cursor() as curs:
            curs.execute(sql_query)
    ...
    for i in range(interval_length):
        ...
        for poly in tqdm(poly_fetch_all, desc="Processing Farmplots"):
            ...
            update
                {table["schema"]}.{table["table"]}
            set
                hue_mean_{args.year}_{args.interval}_{i+1} = {mean},
                hue_stddev_{args.year}_{args.interval}_{i+1} = {stddev}
            where
                {table["key"]} = {poly[0]}
            """
            with pgconn.cursor() as curs:
